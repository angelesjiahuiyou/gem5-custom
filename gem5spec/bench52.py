    ...
    if spawn_list:
        if args.sge:
            ...
            for elem in spawn_list:
                sp_list_aux.append(elem)
                execute(sp_list_aux, args, sem, mode == "cpt_sim")
                del sp_list_aux[:]
    else:
...
def main():
    ...
    parser.add_argument("--l3-hwp", action="store", type=str, default=None,
        ...
        " (default: %(default)s)")
    parser.add_argument("--l3-rp-type", action="store", type=str, default="LRURP",#choices=list(simparams.hwp_config),
            help="L3 replacement policy" + " (default: LRURP)")
