...
class Simulation(object):
    ...
    def setDetailedParams(self, model, tech, case, args):
        ...
        if hier[3] != "none":
            ...
            if args.l3_rp_type:
                self._params["l3-replacement-policy"] = args.l3_rp_type
            if l3p:
...
class CptGeneration(Simulation):
    ...
    def prepareEnvironment(self, benchsuite, args):
        ...
        assert os.path.isdir(self._data_path), "missing folder %s" % (
            self._data_path)
        sp_fname  = "simpointb_%s" % self._wl_ss
        wgt_fname = "weightb_%s" % self._wl_ss
...
class CptSimulation(Simulation):
    ...
    #    if args.l3_rp_type:
   #         out_sim_dir = os.path.join(self._out_path, args.l3_rp_type)
  #      else:
 #           out_sim_dir = self._out_path
#        self._out_path = out_sim_dir
    ...
    def prepareEnvironment(self, benchsuite, args):
        ...
        if args.l3_rp_type:
            out_sim_dir = os.path.join(self._out_path, "cpt_orig" +args.l3_rp_type)
        else:
            out_sim_dir = self._out_path
        self._out_path = out_sim_dir
        cpt_paths = []
...
class FullSimulation(Simulation):
    def __init__(self, args):
        ...
        self._target_dir   = "simulation"
        if args.l3_rp_type:
            out_sim_dir = "full_" + args.l3_rp_type
        else:
            out_sim_dir = "full"
        self._trailing_dir = "full"
        self._trailing_dir = out_sim_dir
        cond_append(self._debug_flags, args.atrace, "AccessTrace")
        cond_append(self._debug_flags, args.ctrace, "ConflictTrace")
        return
