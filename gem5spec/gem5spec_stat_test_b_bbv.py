    ...
    for b_name in args.benchmarks:
        ...
        for subset in ss_params:
            ...
            log_filepath = os.path.join(out_dir, b_abbr + "." + subset[0] +
                ".log")
            if "627" in bbv_filepath:
                stack_str = ' --main-stacksize=134217728 '
            else:
                stack_str = ''
            ...
            if not args.use_gem5:
                ...
                cmd = ("valgrind --tool=exp-bbv --bb-out-file=" +
                    bbv_filepath +  " --interval-size=" + str(args.int_size) +
                    " --pc-out-file=" + pc_filepath + stack_str + " ./" +
                    b_exe_name + " " + subset[1])
                in_name = subset[2]
            else:
...
def sp_gen(args, sem):
    ...
    for b_name in args.benchmarks:
        ...
        for subset in ss_params:
            ...
            sp_filepath = os.path.join(out_dir, "simpointb_" + subset[0])
            wgt_filepath = os.path.join(out_dir, "weightb_" + subset[0])
            log_filepath = os.path.join(out_dir, "logb_" + subset[0])
